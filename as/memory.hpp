...
namespace { namespace memory {
	...
	inline std::string get_base_from_address(std::uint64_t address)
	{
		if (!address)
		...
		for (int j = 0; j < 3; j++)
		{
			for (LIST_ENTRY* pListEntry = ModuleLists[j]->Flink; pListEntry != ModuleLists[j]; pListEntry = pListEntry->Flink)
			{
				...
				std::uint64_t DllBase = (std::uint64_t)pEntry->DllBase;
				PIMAGE_DOS_HEADER DosHeader = (PIMAGE_DOS_HEADER)DllBase;
				PIMAGE_NT_HEADERS NtHeader = (PIMAGE_NT_HEADERS)(DllBase + DosHeader->e_lfanew);
				DWORD SizeOfImage = NtHeader->OptionalHeader.SizeOfImage;

				if (address >= DllBase && address <= (DllBase + SizeOfImage))
				{
					std::wstring unicode_string(pEntry->BaseDllName.Buffer, pEntry->BaseDllName.Length);
	...
	public:
		...
		{
			this->m_address += (value == -1 ? this->m_image : value);
			return *this;
		}
	...
	class vtable
	{
		std::uint64_t m_base_class;
		std::uint64_t* m_vtable;
	public:
		vtable(std::uint64_t base_class)
		{
			this->m_base_class = base_class;
			if (this->m_base_class)
				this->m_vtable = *(std::uint64_t**)this->m_base_class;
		}

		template<const std::uint32_t Index, typename T, typename ... Args>
		inline T call(Args ... args)
		{
			std::uint64_t vtable_func = m_vtable[Index];
			if (!vtable_func) return T();

			return (reinterpret_cast<T(*)(Args...)>(vtable_func))(args ...);
		}

		template<const std::uint32_t Index, const std::uint32_t Size>
		inline bool swap(void* detour, void** original, bool remove_allocation = false)
		{
			if (!detour || !original) return false;

			std::uint64_t* vtable_swap = new std::uint64_t[Size];
			for (int i = 0; i < Size; i++)
				vtable_swap[i] = m_vtable[i];

			*original = *(void**)&m_vtable[Index];
			vtable_swap[Index] = (std::uint64_t)detour;

			_memcpy(m_vtable, vtable_swap, Size);
	...
	/*
	* Example:
	memory::vtable vt((std::uint64_t)gui::d3d11_device_context);
	vt.call<33, bool>("test");
	*/

	inline void* get_unhooked_ptr(void* ptr, int offset = 0x0)
	{
		// F1 (exception hook)
		if (*(uint8_t*)ptr == 0xF1)
		{
			while (*(uint8_t*)ptr == 0xF1)
				ptr = (void*)((uintptr_t)ptr + 0x1);

			return ptr;
		}
		// E9 (jmp hook) (To-do: remove the need of offset)
		else if (*(uint8_t*)ptr == 0xE9)
		{
			auto jmpBytes = (uint8_t*)ptr;
			int jmpOffset = *(int*)&jmpBytes[1];

			uintptr_t jmpLocation = ((uintptr_t)ptr + jmpOffset);
			while (jmpLocation--)
			{
				auto bytes = (uint8_t*)jmpLocation;
				if (bytes[0] == 0xFF && bytes[1] == 0x25)
					return (void*)((uintptr_t)jmpLocation - offset);
			}
		}
