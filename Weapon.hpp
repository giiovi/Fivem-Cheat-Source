...
namespace Weapon
{
	void Hook(void)
	{
		...
		if (Settings::Weapon::DamageBoost) //Working
			WeaponInfo->SetBulletDamage(Settings::Weapon::DamageBoost_value);

		if (Settings::Weapon::InfiniteAmmo) //Working
		...
		*/
		...
		{
			WeaponInfo->SetReload(Settings::Weapon::ReloadSpeed);
		}
		...
		if (Settings::Weapon::SetAmmo != false)
		{
			FixedAmmoCount->SetAmmo(Settings::Weapon::AmmoAmount);
