...
namespace Veh
{
	void Hook(void)
	{
		if (Settings::vehicle::vehicleenable)
		{
			...
			auto trying_to_open_veicle = PED::GET_VEHICLE_PED_IS_TRYING_TO_ENTER(player); //dtc
			auto current_vehicle = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), true); //dtc

			if (Settings::vehicle::unlockcar)
			{
				VEHICLE::SET_VEHICLE_DOORS_LOCKED(trying_to_open_veicle, 1); //dtc
			}

			if (Settings::vehicle::autostart_vehicle)
			{
				VEHICLE::SET_VEHICLE_ENGINE_ON(current_vehicle, true, true, false); //dtc
			}


			if (Settings::vehicle::warp_vehicle)
			{
				PED::SET_PED_INTO_VEHICLE(player, trying_to_open_veicle, -1); //dtc
			}

			if (Settings::vehicle::godmode)
			{

				ENTITY::SET_ENTITY_INVINCIBLE(current_vehicle, true); //look
			...
			if (PED::IS_PED_IN_ANY_VEHICLE(player, false))
			{
				...
				if (Settings::vehicle::repaire)
				{
					...
					VEHICLE::SET_VEHICLE_DEFORMATION_FIXED(current_vehicle); //dtc
					VEHICLE::SET_VEHICLE_ENGINE_HEALTH(current_vehicle, 1000.0);
				...
				{
					VEHICLE::SET_VEHICLE_MOD(current_vehicle, Settings::vehicle::mod_idx, Settings::vehicle::add_idx, false); //dtc
				...
				if (Settings::vehicle::vehicle_speed_bool)
				{
					...
					ActualVehicle->VehicleHandlingInfo()->SetAcceleration(Settings::vehicle::vehicle_speed_value); //dtc

				}
				else
				{
					ActualVehicle->VehicleHandlingInfo()->SetAcceleration(1);
				}
				...
				{
					VEHICLE::SET_VEHICLE_COLOURS(current_vehicle, Settings::vehicle::primary_color, Settings::vehicle::secondary_color); //dtc
				...
				if (Settings::vehicle::update_plate)
				{
					...
					VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(current_vehicle, Settings::vehicle::plate_text); //dtc
